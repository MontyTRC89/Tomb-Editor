...
{
	private static PublishDiagnosticsParams CreateDiagnostics(string filePath, int version, int startCharacter, int endCharacter, string message)
		=> new(
	...
	private static int GetTrackedDocumentCount(LuaLanguageServerIntellisenseProvider provider)
	{
		...
		var manager = (LuaIntellisenseDocumentManager)(field.GetValue(provider)
			?? throw new InvalidOperationException("Provider document manager was null."));

		return manager.TrackedDocumentCount;
	}
	...
	private sealed class FakeLanguageServerClient : ILanguageServerClient
	{
