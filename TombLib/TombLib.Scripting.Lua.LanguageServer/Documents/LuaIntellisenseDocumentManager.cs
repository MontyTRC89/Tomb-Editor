...
public sealed class LuaIntellisenseDocumentManager
{
	...
	/// removed once the reference count drops to zero, so multiple editor tabs sharing the same
	/// file do not invalidate each other on close. When the server-side document is still open,
	/// <paramref name="document"/> contains the snapshot that should be mirrored with <c>didClose</c>.
	/// If a restart is pending and the server copy is already gone, the document is still removed
	/// locally but <paramref name="document"/> is <see langword="null"/>.
	/// </summary>
	/// <param name="filePath">The normalized file path.</param>
	...
	public bool TryClose(string filePath, out LuaDocumentSnapshot? document)
	{
		lock (_syncRoot)
		{
			...
			document = state.IsOpen ? CreateSnapshot(state) : null;
	...
	/// <summary>
	/// Marks every tracked document as closed on the server after a language-server restart. Documents
	/// that still have live editor references are returned so the provider can reopen them eagerly on
	/// the next successful start, while request-only tracked documents reopen lazily on demand.
	/// </summary>
