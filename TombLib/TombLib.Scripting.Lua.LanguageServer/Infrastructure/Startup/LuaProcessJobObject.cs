...
/// processes if the host never gets a chance to run its disposal path.
/// </summary>
	...
	public static void TryAssignProcess(Process process)
	{
		...
		try
		{
			if (!AssignProcessToJobObject(jobHandle, process.Handle))
			{
				...
				// 5 = ERROR_ACCESS_DENIED. Pre-Windows 8 systems or processes already inside an
				// unbreakable job will return this; we just log and move on.
