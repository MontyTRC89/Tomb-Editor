...
/// <summary>
/// Watches the Lua workspace root for relevant external file changes (<c>*.lua</c>, <c>.luarc.json</c>,
/// <c>.luarc.jsonc</c>) and forwards them to LuaLS through <c>workspace/didChangeWatchedFiles</c>.
/// LuaLS does not poll the file system itself for clients that opt into this capability,
/// so without this hook changes made by Git pull, file copy, or any other out-of-band tool would not reach
/// the language server until the user touched the affected document inside the host editor.
/// </summary>
internal sealed class LuaWorkspaceFileWatcher : IDisposable
{
	...
	public LuaWorkspaceFileWatcher(
