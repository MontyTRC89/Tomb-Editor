...
public static partial class LuaLanguageServerResponseParser
{
	...
	/// </summary>
	public static LuaSignatureInfo? ParseSignatureHelp(LuaSignatureHelpResponse? response)
	{
		...
		LuaSignatureHelpSignaturePayload signatureElement = signatures[activeSignature];
		...
		if (signatureElement.Parameters is { Length: > 0 } parametersElement)
		{
			...
			{
				LuaSignatureHelpParameterPayload paramElement = parametersElement[i];
	...
	private static int ResolveActiveParameter(LuaSignatureHelpResponse response, LuaSignatureHelpSignaturePayload signatureElement)
	{
