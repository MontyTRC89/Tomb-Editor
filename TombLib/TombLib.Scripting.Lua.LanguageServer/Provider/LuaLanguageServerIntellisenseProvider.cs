...
public sealed partial class LuaLanguageServerIntellisenseProvider : ILuaIntellisenseProvider
{
	...
	public void OpenDocument(string filePath, string content)
	{
		...
		ObserveBackgroundTask(SynchronizeDocumentAsync(normalizedFilePath, content, acquireOpenReference: true,
	...
	public void UpdateDocument(string filePath, string content)
	{
		...
		ObserveBackgroundTask(SynchronizeDocumentAsync(normalizedFilePath, content, acquireOpenReference: false,
	...
	private async Task DispatchWorkspaceFileChangesAsync(FileChangeBatch batch, CancellationToken cancellationToken)
	{
		...
		var changes = new List<LuaFileEventPayload>(batch.Count);
		...
		foreach ((string path, FileChangeKind kind) in batch.Entries)
		{
			...
			changes.Add(new LuaFileEventPayload(LuaLanguageServerPathHelper.CreateFileUri(normalizedPath), (int)kind));
		}
		...
		try
		{
			...
			if (shouldRefreshConfiguration)
			{
				await _client.SendNotificationAsync("workspace/didChangeConfiguration",
					new LuaDidChangeConfigurationParams(LuaLanguageServerSettingsFactory.Create(_workspaceRootDirectoryPath)),
					cancellationToken).ConfigureAwait(false);
			}

			await _client.SendNotificationAsync("workspace/didChangeWatchedFiles",
				new LuaDidChangeWatchedFilesParams([.. changes]), cancellationToken).ConfigureAwait(false);
		}
