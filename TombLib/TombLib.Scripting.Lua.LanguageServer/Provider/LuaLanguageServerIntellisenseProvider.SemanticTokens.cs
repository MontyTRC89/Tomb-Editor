...
public sealed partial class LuaLanguageServerIntellisenseProvider
{
	...
	private async Task RefreshSemanticTokensAsync(LuaDocumentSnapshot document, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			LuaSemanticTokensWireResponse? response = await SendSemanticTokensRequestAsync(document, deltaState.PreviousResultId, useDelta, effectiveToken)
			...
			if (decodeResult.RetryWithFullRefresh)
			{
				...
				LuaSemanticTokensWireResponse? fullResponse = await SendSemanticTokensRequestAsync(document, previousResultId: null, useDelta: false, effectiveToken)
					.ConfigureAwait(false);
	...
	private Task<LuaSemanticTokensWireResponse?> SendSemanticTokensRequestAsync(
	...
	{
		if (_client is null)
			return Task.FromResult<LuaSemanticTokensWireResponse?>(null);

		string method = useDelta ? "textDocument/semanticTokens/full/delta" : "textDocument/semanticTokens/full";
		var parameters = new LuaSemanticTokensParams(
			new LuaTextDocumentIdentifier(document.Uri),
			useDelta ? previousResultId : null);

		return SendBoundedRequestAsync<LuaSemanticTokensWireResponse?>(_client, method, parameters, timeoutValue: null, cancellationToken);
	}

	private LuaSemanticTokensDecodeResult DecodeSemanticTokensResponse(
		LuaSemanticTokensWireResponse? response, LuaDocumentSnapshot document, int[]? previousData, bool deltaWasRequested)
	{
