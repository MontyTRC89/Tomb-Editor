...
LevelFuncs.Engine.Node.VolumeLight = function(volume, lightcolor, range, effectOffset, shadow, name)

	local entityPos = TEN.Objects.GetVolumeByName(volume):GetPosition()
	effectOffset = effectOffset or Vec3(0, 0, 0)
	local offset = (entityPos) + (effectOffset)
	TEN.Effects.EmitLight(offset, lightcolor, range, shadow, name)
end
...
LevelFuncs.Engine.Node.VolumeFogBulb = function(volume, lightcolor, radius, density, effectOffset, name)
