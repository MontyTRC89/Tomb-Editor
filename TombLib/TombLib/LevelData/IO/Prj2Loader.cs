                ...
                foreach (Room room in newRooms.Values)
                    for (int x = room.LocalArea.X0; x <= room.LocalArea.X1; x++)
                        for (int z = room.LocalArea.Y0; z <= room.LocalArea.Y1; z++)
                        {
                            ...
                            if (usesLegacyFloor)
                            {
                                room.SwapFloor2FacesWhereApplicable(x, z);
                                room.SwapDiagonalFloor2FacesWhereApplicable(x, z);
                            }

                            if (usesLegacyCeiling)
                            {
                                room.SwapCeiling2FacesWhereApplicable(x, z);
                                room.SwapDiagonalCeiling2FacesWhereApplicable(x, z);
                            }
                        }
            }
