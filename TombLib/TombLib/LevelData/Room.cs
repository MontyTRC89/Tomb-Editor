                ...
                for (int z = 0; z < NumZSectors; ++z)
                    for (int x = 0; x < NumXSectors; ++x)
                        foreach (SectorFace face in Sectors[x, z].GetFaceTextures().Keys)
                        {
                            ...
                            var doubleSided = Level.Settings.GameVersion.IsGreaterThan(TRVersion.Game.TR2) && texture.DoubleSided;
                            var copyFace = Level.Settings.GameVersion.IsLessThanOrEqual(TRVersion.Game.TR2) && texture.DoubleSided;
                ...
                foreach (var mesh in meshes)
                {
                    ...
                    foreach (var submesh in mesh.Submeshes)
                    {
                        for (int j = 0; j < submesh.Value.Indices.Count; j += 3)
                        {
                            ...
                            var doubleSided = Level.Settings.GameVersion.IsGreaterThan(TRVersion.Game.TR2) && submesh.Key.DoubleSided;
                            var copyFace = Level.Settings.GameVersion.IsLessThanOrEqual(TRVersion.Game.TR2) && submesh.Key.DoubleSided;
