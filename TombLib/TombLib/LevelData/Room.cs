...
namespace TombLib.LevelData
{
    ...
    public class Room : ITriggerParameter
    {
        ...
        private Attractor TryGenerateNegativeZAttractorForSector(int x, int z)
        {
            RoomSectorPair neighbourSectorPair = GetSectorTryThroughPortal(x, z - 1);

            Sector targetSector = GetSectorTry(x, z),
                neighbourSector = neighbourSectorPair.Sector;

            if (targetSector == null || neighbourSector == null)
                return null;

            (int targetA, int targetB) = (targetSector.Floor.XnZn, targetSector.Floor.XpZn);
            (int neighbourA, int neighbourB) = (neighbourSector.Floor.XnZp, neighbourSector.Floor.XpZp);

            if (targetSector.Floor.DiagonalSplit is DiagonalSplit.XnZp or DiagonalSplit.XpZp) // Target sector is a diagonal wall and the floor piece is on the NegativeZ side
                targetA = targetB = Math.Min(targetSector.Floor.XnZn, targetSector.Floor.XpZn); // Use the lower of the two floor heights

            if (neighbourSector.Floor.DiagonalSplit is DiagonalSplit.XnZn or DiagonalSplit.XpZn) // Neighbour sector is a diagonal wall and the floor piece is on the PositiveZ side
                neighbourA = neighbourB = Math.Min(neighbourSector.Floor.XnZp, neighbourSector.Floor.XpZp); // Use the lower of the two floor heights

            if (targetSector.Type != SectorType.Floor) // Target sector is not floor
            {
                if (targetSector.Floor.DiagonalSplit == DiagonalSplit.None) // Target sector is not a diagonal wall (which has a floor piece)
                    return null;

                if (targetSector.Floor.DiagonalSplit is DiagonalSplit.XnZn or DiagonalSplit.XpZn) // Target sector is a diagonal wall, but the floor piece is not on the NegativeZ side
                    return null;
            }

            if (neighbourSector.Type != SectorType.Floor)
            {
                if (neighbourSector.Floor.DiagonalSplit == DiagonalSplit.None) // Neighbour sector is not a diagonal wall (which has a floor piece)
                    return null;

                if (neighbourSector.Floor.DiagonalSplit is DiagonalSplit.XnZp or DiagonalSplit.XpZp) // Neighbour sector is a diagonal wall, but the floor piece is not on the PositiveZ side
                    return null;
            }

            if (targetSector.FloorPortal != null)
            {
                ...
                {
                    if (targetA + Position.Y <= neighbourA + neighbourSectorPair.Room.Position.Y && targetB + Position.Y <= neighbourB + neighbourSectorPair.Room.Position.Y)
                        return null;
                }
            }
            else if (targetSector.FloorPortal == null && neighbourSector.FloorPortal != null)
            {
                if (targetA + Position.Y < neighbourA + neighbourSectorPair.Room.Position.Y && targetB + Position.Y < neighbourB + neighbourSectorPair.Room.Position.Y)
                    return null;
            }
            else if (targetA + Position.Y <= neighbourA + neighbourSectorPair.Room.Position.Y && targetB + Position.Y <= neighbourB + neighbourSectorPair.Room.Position.Y) // If target sector is not higher than the neighbour sector
                return null;

            return new Attractor(AttractorType.Edge, new(x, targetA, z), new(x + 1, targetB, z));
        }

        private Attractor TryGeneratePositiveZAttractorForSector(int x, int z)
        {
            RoomSectorPair neighbourSectorPair = GetSectorTryThroughPortal(x, z + 1);
            ...
            (int targetA, int targetB) = (targetSector.Floor.XpZp, targetSector.Floor.XnZp);
            (int neighbourA, int neighbourB) = (neighbourSector.Floor.XpZn, neighbourSector.Floor.XnZn);

            if (targetSector.Floor.DiagonalSplit is DiagonalSplit.XnZn or DiagonalSplit.XpZn) // Target sector is a diagonal wall and the floor piece is on the PositiveZ side
                targetA = targetB = Math.Min(targetSector.Floor.XnZp, targetSector.Floor.XpZp); // Use the lower of the two floor heights

            if (neighbourSector.Floor.DiagonalSplit is DiagonalSplit.XnZp or DiagonalSplit.XpZp) // Neighbour sector is a diagonal wall and the floor piece is on the NegativeZ side
                neighbourA = neighbourB = Math.Min(neighbourSector.Floor.XnZn, neighbourSector.Floor.XpZn); // Use the lower of the two floor heights
            ...
            if (targetSector.Type != SectorType.Floor) // Target sector is not floor
            {
                ...
                if (targetSector.Floor.DiagonalSplit is DiagonalSplit.XnZp or DiagonalSplit.XpZp) // Target sector is a diagonal wall, but the floor piece is not on the PositiveZ side
                    return null;
            }
            ...
            if (neighbourSector.Type != SectorType.Floor)
            {
                ...
                if (neighbourSector.Floor.DiagonalSplit is DiagonalSplit.XnZn or DiagonalSplit.XpZn) // Neighbour sector is a diagonal wall, but the floor piece is not on the NegativeZ side
                    return null;
            }
        ...
        private Attractor TryGenerateNegativeXAttractorForSector(int x, int z)
        {
            RoomSectorPair neighbourSectorPair = GetSectorTryThroughPortal(x - 1, z);
            ...
            (int targetA, int targetB) = (targetSector.Floor.XnZp, targetSector.Floor.XnZn);
            (int neighbourA, int neighbourB) = (neighbourSector.Floor.XpZp, neighbourSector.Floor.XpZn);

            if (targetSector.Floor.DiagonalSplit is DiagonalSplit.XpZp or DiagonalSplit.XpZn) // Target sector is a diagonal wall and the floor piece is on the PositiveX side
                targetA = targetB = Math.Min(targetSector.Floor.XnZp, targetSector.Floor.XnZn); // Use the lower of the two floor heights

            if (neighbourSector.Floor.DiagonalSplit is DiagonalSplit.XnZp or DiagonalSplit.XnZn) // Neighbour sector is a diagonal wall and the floor piece is on the NegativeX side
                neighbourA = neighbourB = Math.Min(neighbourSector.Floor.XpZp, neighbourSector.Floor.XpZn); // Use the lower of the two floor heights
            ...
            if (targetSector.Type != SectorType.Floor) // Target sector is not floor
            {
                ...
                if (targetSector.Floor.DiagonalSplit is DiagonalSplit.XnZp or DiagonalSplit.XnZn) // Target sector is a diagonal wall, but the floor piece is not on the PositiveX side
                    return null;
            }
            ...
            if (neighbourSector.Type != SectorType.Floor)
            {
                ...
                if (neighbourSector.Floor.DiagonalSplit is DiagonalSplit.XpZp or DiagonalSplit.XpZn) // Neighbour sector is a diagonal wall, but the floor piece is not on the NegativeX side
                    return null;
            }
        ...
        private Attractor TryGeneratePositiveXAttractorForSector(int x, int z)
        {
            RoomSectorPair neighbourSectorPair = GetSectorTryThroughPortal(x + 1, z);
            ...
            (int targetA, int targetB) = (targetSector.Floor.XpZn, targetSector.Floor.XpZp);
            (int neighbourA, int neighbourB) = (neighbourSector.Floor.XnZn, neighbourSector.Floor.XnZp);

            if (targetSector.Floor.DiagonalSplit is DiagonalSplit.XnZn or DiagonalSplit.XnZp) // Target sector is a diagonal wall and the floor piece is on the NegativeX side
                targetA = targetB = Math.Min(targetSector.Floor.XpZn, targetSector.Floor.XpZp); // Use the lower of the two floor heights

            if (neighbourSector.Floor.DiagonalSplit is DiagonalSplit.XpZn or DiagonalSplit.XpZp) // Neighbour sector is a diagonal wall and the floor piece is on the PositiveX side
                neighbourA = neighbourB = Math.Min(neighbourSector.Floor.XnZn, neighbourSector.Floor.XnZp); // Use the lower of the two floor heights
            ...
            if (targetSector.Type != SectorType.Floor) // Target sector is not floor
            {
                ...
                if (targetSector.Floor.DiagonalSplit is DiagonalSplit.XpZn or DiagonalSplit.XpZp) // Target sector is a diagonal wall, but the floor piece is not on the NegativeX side
                    return null;
            }
            ...
            if (neighbourSector.Type != SectorType.Floor)
            {
                ...
                if (neighbourSector.Floor.DiagonalSplit is DiagonalSplit.XnZn or DiagonalSplit.XnZp) // Neighbour sector is a diagonal wall, but the floor piece is not on the PositiveX side
                    return null;
            }
        ...
        private Attractor TryGenerateDiagonalXnZpAttractorForSector(int x, int z)
        {
            Sector sector = GetSectorTry(x, z);

            if (sector == null || sector.Type != SectorType.Floor || sector.Floor.DiagonalSplit != DiagonalSplit.XnZp)
            {
                if (sector?.FloorPortal != null)
                {
                    ...
                    return new Attractor(AttractorType.Edge, new(x, sector.Floor.XnZn, z), new(x + 1, sector.Floor.XpZp, z + 1));
                }
                else
                    return null;
            }   

            (int targetA, int targetB) = (sector.Floor.XnZn, sector.Floor.XpZp);

            if (targetA <= sector.Floor.XpZn && targetB <= sector.Floor.XpZn)
                return null;

            return new Attractor(AttractorType.Edge, new(x, targetA, z), new(x + 1, targetB, z + 1));
        }

        private Attractor TryGenerateDiagonalXpZpAttractorForSector(int x, int z)
        {
            Sector sector = GetSectorTry(x, z);

            if (sector == null || sector.Type != SectorType.Floor || sector.Floor.DiagonalSplit != DiagonalSplit.XpZp)
            {
                if (sector?.FloorPortal != null)
                {
                    RoomConnectionInfo floorPortalInfo = GetFloorRoomConnectionInfo(new VectorInt2(x, z));
                    ...
                    return new Attractor(AttractorType.Edge, new(x, sector.Floor.XnZp, z + 1), new(x + 1, sector.Floor.XpZn, z));
                }
                else
                    return null;
            }

            (int targetA, int targetB) = (sector.Floor.XnZp, sector.Floor.XpZn);

            if (targetA <= sector.Floor.XnZn && targetB <= sector.Floor.XnZn)
                return null;

            return new Attractor(AttractorType.Edge, new(x, targetA, z + 1), new(x + 1, targetB, z));
        }

        private Attractor TryGenerateDiagonalXpZnAttractorForSector(int x, int z)
        {
            Sector sector = GetSectorTry(x, z);

            if (sector == null || sector.Type != SectorType.Floor || sector.Floor.DiagonalSplit != DiagonalSplit.XpZn)
            {
                if (sector?.FloorPortal != null)
                {
                    RoomConnectionInfo floorPortalInfo = GetFloorRoomConnectionInfo(new VectorInt2(x, z));
                    ...
                    return new Attractor(AttractorType.Edge, new(x + 1, sector.Floor.XpZp, z + 1), new(x, sector.Floor.XnZn, z));
                }
                else
                    return null;
            }
            
            (int targetA, int targetB) = (sector.Floor.XpZp, sector.Floor.XnZn);

            if (targetA <= sector.Floor.XnZp && targetB <= sector.Floor.XnZp)
                return null;

            return new Attractor(AttractorType.Edge, new(x + 1, targetA, z + 1), new(x, targetB, z));
        }

        private Attractor TryGenerateDiagonalXnZnAttractorForSector(int x, int z)
        {
            Sector sector = GetSectorTry(x, z);

            if (sector == null || sector.Type != SectorType.Floor || sector.Floor.DiagonalSplit != DiagonalSplit.XnZn)
            {
                if (sector?.FloorPortal != null)
                {
                    RoomConnectionInfo floorPortalInfo = GetFloorRoomConnectionInfo(new VectorInt2(x, z));
                    ...
                    return new Attractor(AttractorType.Edge, new(x + 1, sector.Floor.XpZn, z), new(x, sector.Floor.XnZp, z + 1));
                }
                else
                    return null;
            }
            
            (int targetA, int targetB) = (sector.Floor.XpZn, sector.Floor.XnZp);

            if (targetA <= sector.Floor.XpZp && targetB <= sector.Floor.XpZp)
                return null;
        ...
        public void GenerateAttractors()
        {
            ...
            for (int z = 0; z < NumZSectors; ++z)
                ...
                {
                    AddIfNotNull(TryGenerateNegativeZAttractorForSector(x, z));
                    AddIfNotNull(TryGenerateNegativeXAttractorForSector(x, z));
                    AddIfNotNull(TryGeneratePositiveZAttractorForSector(x, z));
                    AddIfNotNull(TryGeneratePositiveXAttractorForSector(x, z));
                    AddIfNotNull(TryGenerateDiagonalXnZpAttractorForSector(x, z));
                    AddIfNotNull(TryGenerateDiagonalXpZpAttractorForSector(x, z));
                    AddIfNotNull(TryGenerateDiagonalXpZnAttractorForSector(x, z));
                    AddIfNotNull(TryGenerateDiagonalXnZnAttractorForSector(x, z));
                }
