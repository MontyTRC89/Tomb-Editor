    ...
    public partial class LevelCompilerClassicTR
    {
        private void WriteLevelTr1()
        {
            ...
            using (var writer = new BinaryWriter(new FileStream(_dest, FileMode.Create, FileAccess.Write, FileShare.None)))
            {
                ...
                {
                    writer.Write(_boxes[i].Zmin * (int)Level.SectorSizeUnit);
                    writer.Write(_boxes[i].Zmax * (int)Level.SectorSizeUnit);
                    writer.Write(_boxes[i].Xmin * (int)Level.SectorSizeUnit);
                    writer.Write(_boxes[i].Xmax * (int)Level.SectorSizeUnit);
