...
namespace TombLib.LevelData.Compilers
{
    public sealed partial class LevelCompilerClassicTR
    {
        ...
        private TextureFootStep.Type GetTextureSound(Room room, int x, int z)
        {
            ...
            TextureFootStep.Type? result0 = GetTextureSound(!sector.Floor.IsQuad, sector.GetFaceTexture(new(SectorFace.Floor, FaceLayer.Base)));
            if (result0.HasValue)
                return result0.Value;

            TextureFootStep.Type? result1 = GetTextureSound(!sector.Floor.IsQuad, sector.GetFaceTexture(new(SectorFace.Floor_Triangle2, FaceLayer.Base)));
            if (result1.HasValue)
        ...
        private byte[] CalculateLightmap(tr_color[] palette)
        {
            ...
            for (int k = 0; k < 32; k++)
            {
                var bias  = _level.Settings.GameVersion.Native() == TRVersion.Game.TR1 ? k + 1 : k;
