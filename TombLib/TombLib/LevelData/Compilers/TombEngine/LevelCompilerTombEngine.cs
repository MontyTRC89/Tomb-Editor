...
namespace TombLib.LevelData.Compilers.TombEngine
{
    public sealed partial class LevelCompilerTombEngine : LevelCompiler
    {
        ...
        private void WriteTriggerNodeEvents()
        {
            var luaPath = _level.Settings.MakeAbsolute(_level.Settings.TenLuaScriptFile);

            if (!File.Exists(luaPath))
                return;

            ReportProgress(99, "\nCompiling trigger node events...");

            var levelScriptName = Path.GetFileNameWithoutExtension(luaPath) + "_Nodes.lua";
            var scriptDirectory = Path.GetDirectoryName(luaPath);
            var nodeScriptName = Path.Combine(scriptDirectory, levelScriptName);
            ...
            {
                if (set.OnEnter.Nodes.Count > 0)
                {
                    result += ScriptingUtils.ParseNodes(set.OnEnter.Nodes, set.OnEnter.GenerateFunctionName(_level.Settings.EventSets));
                    result += Environment.NewLine + Environment.NewLine;
                    count++;
                }

                if (set.OnInside.Nodes.Count > 0)
                {
                    result += ScriptingUtils.ParseNodes(set.OnInside.Nodes, set.OnInside.GenerateFunctionName(_level.Settings.EventSets));
                    result += Environment.NewLine + Environment.NewLine;
                    count++;
                }

                if (set.OnLeave.Nodes.Count > 0)
                {
                    result += ScriptingUtils.ParseNodes(set.OnLeave.Nodes, set.OnLeave.GenerateFunctionName(_level.Settings.EventSets));
                    result += Environment.NewLine + Environment.NewLine;
                    count++;
                }
            }
            ...
            if (count > 0)
            {
                ReportProgress(99, "Number of compiled trigger node events: " + count);
                ReportProgress(99, "Writing trigger node event script to " + nodeScriptName);

                var comment = "-- Trigger node script, autogenerated by Tomb Editor " + Application.ProductVersion + Environment.NewLine +
                              "-- Do not modify this file manually." + Environment.NewLine + Environment.NewLine;
