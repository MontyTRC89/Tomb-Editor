...
namespace TombLib.LevelData.Compilers.TombEngine
{
	...
	room.Sectors[x, z].FloorPortal.AdjoiningRoom.Properties.Type == RoomType.Water)
										{
											realBlendMode = BlendMode.DynamicWaterSurface;
										}
									...
									else
									{
										...
										if (room.Properties.Type == RoomType.Normal &&
											...
											room.Sectors[x, z].FloorPortal.AdjoiningRoom.Properties.Type == RoomType.Water)
										{
											poly.BlendMode = (byte)BlendMode.DynamicWaterSurface;
										}
		...
		private void ConvertFloorCeilingPortal(Room room, PortalInstance portal, List<TombEnginePortal> outPortals, bool isCeiling)
		{
			...
			for (int i = 0; i < portalPlanes.Count; ++i)
			{
					...
					portal.AdjoiningRoom.Properties.Type == RoomType.Water)
				{
					...
					waterPlane.Y = portalVertices[0].Y;
					waterPlane.XMin = portalVertices.Min(v => v.X);
					waterPlane.XMax = portalVertices.Max(v => v.X);
		...
		private void PrepareWaterPlanesAndAssignToBuckets()
		{
			for (int i = 0; i < _tempRooms.Count; i++)
			{
				TombEngineRoom room = _tempRooms.ElementAt(i).Value;
				if (room != null)
				{
					foreach (var bucket in room.Buckets)
					{
						if (bucket.Key.BlendMode == (byte)BlendMode.DynamicWaterSurface)
						{
