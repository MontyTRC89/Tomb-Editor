    ...
    public partial class LevelCompilerClassicTR
    {
        ...
        private void WriteLevelTr5Main()
        {
            ...
            using (var geometryDataStream = new MemoryStream())
            {
                ...
                // In TR5 file format here we have six 0xcd, now here I store my new special data
                writer.Write((int)0x5455354D);

                // Write LUA triggers
                int numLuaTrigger = _luaTriggers.Count;
                writer.Write(numLuaTrigger);
                foreach (var trigger in _luaTriggers)
                {
                    ...
                    string function = "function " + functionName + "()\n" +
                                      trigger.LuaScript + "\n" +
                                      "end\n";
                    writer.Write((int)function.Length);
                    writer.Write(System.Text.Encoding.UTF8.GetBytes(trigger.LuaScript));
                }
