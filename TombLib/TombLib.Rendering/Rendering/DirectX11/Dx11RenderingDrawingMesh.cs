...
namespace TombLib.Rendering.DirectX11
{
    ...
    public sealed unsafe class Dx11RenderingDrawingMesh : RenderingDrawingMesh
    {
        // Layout MUST mirror the MeshData cbuffer in MeshShaderVS.hlsl/PS.hlsl.
        // Total size includes 32 bones x 64 bytes = 2048 bytes plus the 16-byte
        ...
        private struct MeshDataLayout
        {
            ...
            // Bones[MAX_BONES] (matrix array, 32 x 64 = 2048B)
        ...
        private readonly ID3D11Buffer* _meshDataBuffer;
        ...
        public Dx11RenderingDrawingMesh(Dx11RenderingDevice device, Description description)
        {
            _device = device;
            ...
            _vertexBufferBinding = new Dx11VertexBufferBinding(_vertexBuffer, sizeof(MeshVertex), 0);
            ...
            Dx11RenderingDevice.SetDebugName((ID3D11DeviceChild*)_meshDataBuffer, "DrawingMesh.MeshData");
        }
        ...
        public override void Render(RenderArgs arg)
        {
            ...
            ID3D11ShaderResourceView* srv = ResolveAtlasSrv(arg.Atlas);
            if (srv == null)
            ...
            fixed (byte* dst = _cbufferStaging)
            {
                ...
                ctx->UpdateSubresource((ID3D11Resource*)_meshDataBuffer, 0, null, dst, (uint)MeshDataSize, 0);
            }

            // Bind shader, IA, sampler, atlas SRV.
            _device.MeshShader.Apply(ctx, arg.StateBuffer);
            { var b = _meshDataBuffer; ctx->VSSetConstantBuffers(1, 1, &b); }
            { var b = _meshDataBuffer; ctx->PSSetConstantBuffers(1, 1, &b); }
            ...
            foreach (var sub in _submeshes)
            {
                ...
                ID3D11BlendState* wantBlend = sub.AdditiveBlending
        ...
        // native ID3D11ShaderResourceView*; null disables texturing.
        private static ID3D11ShaderResourceView* ResolveAtlasSrv(object atlas)
        {
            if (atlas == null)
                return null;
            if (atlas is nint ptr)
                return (ID3D11ShaderResourceView*)ptr;
            throw new ArgumentException("Atlas must be an nint wrapping ID3D11ShaderResourceView*, got " + atlas.GetType().Name);
        }
    }
}
