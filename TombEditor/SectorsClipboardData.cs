...
namespace TombEditor
{
    ...
    public class SectorsClipboardData
    {
        ...
        {
            SectorSelection selection = editor.SelectedSectors;
            ...
            {
                var sectors = new Block[selection.Area.Width, selection.Area.Height];

                for (int x = 0; x < Width; x++)
                    for (int z = 0; z < Height; z++)
                    {
                        int currX = selection.Area.X0 + x;
                        int currZ = selection.Area.Y0 + z;
                        Block b = editor.SelectedRoom.Blocks[currX, currZ];
        ...
        public Block[,] GetSectors()
        {
            ...
            using (var ms = new MemoryStream(_data))
            {
                ...
                for (int x = 0; x < Width; x++)
                    for (int z = 0; z < Height; z++)
                    {
                        Block b = sectors[x, z] = new Block(0, 12);
