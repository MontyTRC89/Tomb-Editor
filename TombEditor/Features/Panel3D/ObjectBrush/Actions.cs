    ...
    public static class Actions
    {
        ...
        private static void ExecuteBrushAction(Editor editor, Room room, float localX, float localZ)
        {
            ...
            switch (editor.Tool.Tool)
            {
                case EditorToolType.Brush:
                    var placed = Helpers.PlaceObjectsWithBrush(editor, room, localX, localZ, sectorConstraint);
                ...
                case EditorToolType.Eraser:
                    var removed = Helpers.EraseObjectsWithBrush(editor, room, localX, localZ, sectorConstraint);
                ...
                case EditorToolType.ObjectSelection:
                    Helpers.SelectObjectsWithBrush(editor, room, localX, localZ, sectorConstraint, _strokeProcessedObjects);
                    break;

                case EditorToolType.ObjectDeselection:
                    Helpers.SelectObjectsWithBrush(editor, room, localX, localZ, sectorConstraint, _strokeProcessedObjects, deselect: true);
                    break;

                case EditorToolType.Pencil:
                case EditorToolType.Line:
                    var pencilPlaced = Helpers.PlaceObjectWithPencil(editor, room, localX, localZ, ref _pencilItemIndex,
        ...
        public static void ExecuteFill(Editor editor, Room selectedRoom)
        {
            ...
            var sectorConstraint = editor.SelectedSectors.Valid && !editor.SelectedSectors.Empty
                ? editor.SelectedSectors.Area
                : (RectangleInt2?)new RectangleInt2(1, 1, selectedRoom.NumXSectors - 2, selectedRoom.NumZSectors - 2);

            var placed = Helpers.FillAreaWithObjects(editor, selectedRoom, editor.ChosenItems, sectorConstraint.Value);
            if (placed.Count == 0)
                return;
