    ...
    class Effects
    {
        ...
        public static bool Initialize(GraphicsDevice device)
        {
            ...
            catch (Exception exc)
            {
                ...
                MessageBox.Show("Could not load effect file. " + Environment.NewLine + exc.Message, "Error",
        ...
        private static Effect LoadEffect(string name)
        {
            ...
            if (result.HasErrors)
            {
                ...
                logger.Error("Could not compile effect '" + name + ".fx'");
                MessageBox.Show("Could not compile effect '" + name + ".fx'" + Environment.NewLine + errors, "Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                return null;
            }

            var effect = new Effect(GraphicsDevice, result.EffectData);
            return effect;
        }
    }
}
