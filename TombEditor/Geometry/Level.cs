            ...
            try
            {
                ...
                using (var reader = new BinaryReaderEx(File.OpenRead(filename)))
                {
                    ...
                    for (int i = 0; i < MaxNumberOfRooms; ++i)
                        level.Rooms[i] = i >= numRooms ? null : new Room(level);
                    ...
                    for (int i = 0; i < numRooms; i++)
                    {
                        ...
                        for (int j = 0; j < numPortals; j++)
                        {
                            ...
                            reader.ReadInt16();
                            var portalRoom = level.Rooms[reader.ReadInt16()];
                        ...
                        for (int j = 0; j < numObjects; j++)
                        {
                            ...
                            var objRoom = level.Rooms[reader.ReadInt16()];
                    ...
                    for (int i = 0; i < level.Rooms.Length; i++)
                    {
                        ...
                        foreach (var info in flipInfos)
                        {
                            if (info._baseRoom == i)
                            {
                                ...
                                level.Rooms[i].AlternateRoom = level.Rooms[info._flipRoom];
                            ...
                            level.Rooms[i].Flipped = true;
                            level.Rooms[i].BaseRoom = level.Rooms[info._baseRoom];
                            level.Rooms[i].AlternateGroup = info._group;
        ...
        public static Level LoadFromPrj2(string filename, GraphicsDevice device)
        {
            ...
            try
            {
                using (var reader = CreatePrjReader(filename))
                {
                    ...
                    {
                        const string fallbackFile = "coastal.png";
                        logger.Warn($"Texture file {level.TextureFile} does not exist, using fallback {fallbackFile}");
                        level.TextureFile = fallbackFile;
                    }
