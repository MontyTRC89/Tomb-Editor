...
/// <summary>
/// Provides a precomputed table of line start offsets and lengths for a Lua document content string.
...
/// </summary>
internal sealed class LuaDocumentLineOffsets
{
	private readonly string _content;
	private readonly int[] _lineStartOffsets;
	private readonly int[] _lineLengths;

	private LuaDocumentLineOffsets(string content, int[] lineStartOffsets, int[] lineLengths)
	{
		_content = content;
	...
	public static LuaDocumentLineOffsets Build(string? content)
	{
		...
		if (text.Length == 0)
			return new LuaDocumentLineOffsets(text, [0], [0]);
