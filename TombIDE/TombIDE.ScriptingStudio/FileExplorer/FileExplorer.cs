...
namespace TombIDE.ScriptingStudio.FileExplorer
{
	public partial class FileExplorer : DarkToolWindow
	{
		...
		private void menuItem_ViewCode_Click(object sender, EventArgs e) => OpenSelectedFile(openSourceView: true);
		...
		private void OpenSelectedFile(EditorType editorType = EditorType.Default, bool openSourceView = false)
		{
			if (treeView.SelectedNodes.Count == 0)
				return;
			...
			OnFileOpened(openSourceView
				? FileOpenedEventArgs.CreateSourceView(selectedNodeFileInfo.FullName)
				: new FileOpenedEventArgs(selectedNodeFileInfo.FullName, editorType));
		}
