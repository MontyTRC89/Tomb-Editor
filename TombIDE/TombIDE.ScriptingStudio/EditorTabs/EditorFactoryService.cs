...
{
	private readonly ConditionalWeakTable<IEditorControl, EditorRegistration> _editorRegistrations = new();
	...
	public EditorType GetSourceViewEditorType(string filePath)
		=> _registrations.FirstOrDefault(registration => registration.SupportsFile(filePath) && registration.EditorType == EditorType.Text)?.EditorType
	...
	private EditorRegistration? ResolveDefaultRegistration(string filePath)
		=> _registrations.FirstOrDefault(registration => registration.SupportsFile(filePath) && registration.IsDefaultForFile(filePath))
