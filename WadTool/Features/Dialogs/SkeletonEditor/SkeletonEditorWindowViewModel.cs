...
public partial class SkeletonEditorWindowViewModel : ObservableObject, IModalDialogViewModel
{
    ...
    private void RebuildTree()
    {
        ...
        var rootNode = new BoneTreeNode(_bones[0], "0: " + _bones[0].Bone.Name);
        ...
        for (int j = 1; j < _bones.Count; j++)
        {
            ...
            string op = boneNode.Bone.OpCode switch
            {
            ...
            var newNode = new BoneTreeNode(_bones[j], j + ": " + op + _bones[j].Bone.Name);
    ...
    private void RenameBone()
    {
        ...
        if (form.ShowDialog(Owner) == WinForms.DialogResult.OK && form.Result != "")
        {
            ...
            if (_nodesDictionary.TryGetValue(theNode, out BoneTreeNode? node))
                node.Text = node.Text.Contains(": ") ? node.Text[..(node.Text.IndexOf(": ") + 2)] + form.Result : form.Result;
            _panel?.Invalidate();
        }
    }
